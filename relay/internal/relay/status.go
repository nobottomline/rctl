...
func (s *server) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"rootless_update_configured":     s.cfg.RootlessUpdateManifestURL != "",
		"rootless_update_target_version": s.cfg.RootlessUpdateTargetVersion,
		"device_package_available":       len(s.publicPackage) != 0 || len(s.rootlessPackage) != 0,
		"device_packages":                s.devicePackageOptions(),
		"device_package_version":         s.publicPackageInfo.Version,
		"history_retention_seconds":      int64(s.cfg.HistoryRetention.Seconds()),
