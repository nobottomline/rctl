...
/// Port of `mobile/ios/ControllerTests/ControllerLifecycleTests.swift` against
/// the UIKit app's `Core` models. Test names and assertions are unchanged;
/// iOS 16 clock/duration APIs are replaced with iOS 13 equivalents, models
/// that the original built with `ControllerAppModel()` use an isolated model,
/// and positive assertions after a fixed delay await their condition instead.
@MainActor
final class ControllerLifecycleTests: XCTestCase {
    ...
    func testOldRelayAcknowledgementDoesNotCacheUnacceptedSchema() async throws {
        ...
        // Grace period for a write that must not happen (as in the original).
        ...
        while fixture.profiles.reportedClientProfile(relayID: fixture.relayID) == nil, deadline.hasTimeRemaining {
    ...
    func testFreshVideoIsRequiredAndRecoveryNeverRestoresControl() {
        let model = RemoteSessionModel(appModel: makeIsolatedControllerAppModel(), deviceID: "test-device")
    ...
    func testReconnectBudgetAndFreshRequests() async throws {
        ...
        // The original waited a fixed 100 ms for the last failure to land.
        let deadline = CoreTestDeadline(seconds: 3)
        while model.state == .signaling, deadline.hasTimeRemaining {
            try await Task.sleep(seconds: 0.01)
        }
        XCTAssertEqual(model.reconnectAttempt, 3)
        XCTAssertFalse(model.reconnecting)
        XCTAssertEqual(model.state, .failed)
        XCTAssertEqual(CoreRequestStub.requests.count("/v1/capabilities"), 4)
    }
    ...
    func testSuspendCancelsPendingAutomaticReconnect() async throws {
        ...
        model.suspend()
        // Outlast the first 1 s backoff: the cancelled retry must never start.
        try await Task.sleep(seconds: 1.2)
        XCTAssertFalse(model.reconnecting)
        XCTAssertEqual(model.state, .closed)
        XCTAssertEqual(CoreRequestStub.requests.count("/v1/capabilities"), 1)
        model.disconnect()
    }

    private func localRetryRequest() async throws -> CoreRequestStub {
        let deadline = CoreTestDeadline(seconds: 6)
        while deadline.hasTimeRemaining {
            if let pending = CoreRequestStub.requests.take("/v1/capabilities") { return pending }
            try await Task.sleep(seconds: 0.01)
        }
    ...
    func testConcurrentSignalingUsesOneRefresh() async throws {
        ...
        XCTAssertEqual(CoreRequestStub.requests.count("/api/controller/token/refresh"), 2)
    }
    ...
    private func request(_ path: String) async throws -> CoreRequestStub {
        let deadline = CoreTestDeadline(seconds: 3)
        while deadline.hasTimeRemaining {
            if path != "/api/controller/me", let info = CoreRequestStub.requests.take("/api/controller/me") {
                info.respond(info.request.url?.host == "second.example" ? Self.secondControllerInfo : Self.controllerInfo)
            }
            if let request = CoreRequestStub.requests.take(path) { return request }
            try await Task.sleep(seconds: 0.01)
        }
    ...
    func testPresenceCancellationAndOldServerCompatibility() async throws {
        ...
        XCTAssertEqual(CoreRequestStub.requests.count("/api/controller/presence"), 1)
        XCTAssertNil(fixture.model.presentedError)
    ...
    func testDeviceProfileIsReportedOncePerFingerprintAndHeartbeatCarriesTelemetry() async throws {
        ...
        again.cancel()
        await again.value
        XCTAssertEqual(CoreRequestStub.requests.count("/api/controller/me/client"), 1, "Unchanged profile is not re-sent")
    }
...
/// One saved relay (plus an optional second) in a private UserDefaults suite
/// and Keychain namespace, served only by `CoreRequestStub`.
@MainActor
