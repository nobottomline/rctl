...
An explicit selection pauses automatic resolution without cancelling NWBrowser
or clearing its results. It re-resolves DNS for up to twenty seconds (at most
four rounds), allowing a sleeping device to answer on a subsequent wake window,
and asks the system for best-effort WakeOnResolve. No cached endpoint fallback,
subnet scan, automatic wake request, or permanent power assertion is used.
Transient resolve failures retry automatically with 1-30 second backoff;
invalid/unsupported records do not loop. A disappearing service is retained for
thirty seconds as unavailable, with its endpoint cleared, rather than rendered
as online or immediately replaced by an empty-state view. Explicit retry still
requires fresh DNS validation. Backgrounding/Stop clears all retained state.

Sleep qualification (2026-09-10, rootful): a system DNS-SD browser outside the
app observed no removals for ninety seconds while the operator kept the iPad
unlocked. After locking, it repeatedly observed roughly three-second visible
windows separated by roughly twelve-second absences. The daemon stayed alive
with LAN enabled. Thus the removal events are not generated by SwiftUI; the
exact OS/network sleep mechanism remains outside this qualification. Three
consecutive explicit selections using the new resolver and real capabilities
requests passed from macOS while the iPad was locked (35 seconds total).
This does not qualify physical iPhone behavior or guarantee discovery across
sleeping/offline devices. Save a manually confirmed device for persistent access.
