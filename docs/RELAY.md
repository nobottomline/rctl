...
The local LAN/USB server remains independent and enabled by default. Installing
a relay-enabled package does not itself disable or replace
`http://<ipad-ip>:8080` or `http://localhost:8080` over USB forwarding. Only the
explicit Relay-only admin action changes its bind address.
...
LocalAccessEnabled bool  true/default for LAN + relay; false binds HTTP to loopback
...
failure of one does not disconnect the others or change the configured local-access policy. WebRTC session
...
device, the admin device menu can switch between `LAN + Relay` and `Relay only`.
The latter persists `LocalAccessEnabled=false` and restarts `rctld`; its HTTP
service then listens on `127.0.0.1:8080`, which remains reachable by the relay's
on-device tunnel but not by network peers. The device rejects this transition
unless at least one enabled relay entry already contains a long-lived
`DeviceSecret`.

The mutation uses `POST application/json` and a short-lived confirmation token
bound to `local_access:relay-only` or `local_access:lan`. The product UI exposes
it to authenticated relay administrators through the normal device proxy; as
with every local API, a trusted-LAN caller that can already reach port `8080`
